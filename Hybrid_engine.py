...
# ============================================================
def validate_cpt_code(code: Any) -> Dict[str, Any]:
    """
    Validate a CPT or HCPCS code against official category and range rules.

    Category I  (5-digit numeric)
    ─────────────────────────────
    Anesthesia          : 00100–01999, 99100–99150
    Surgery             : 10004–69990
      Integumentary     : 10000–19499
    ...
    Category II  (4 digits + F)   e.g. 0001F  – performance tracking
    Category III (4 digits + T)   e.g. 0042T  – emerging technology
    HCPCS Level II (letter A–V + 4 digits)    e.g. E1399, A0001

    Returns:
        {
            "valid"    : bool,
            "category" : str | None,
            "reason"   : str          # non-empty only when invalid
        }
    """
    if not code:
        return {"valid": False, "category": None, "reason": "Empty code"}

    code_clean = str(code).strip().upper()

    # ── Category II ──────────────────────────────────────────────────────
    if re.match(r'^\d{4}F$', code_clean):
        return {"valid": True, "category": "Category II – Performance", "reason": ""}

    # ── Category III ─────────────────────────────────────────────────────
    if re.match(r'^\d{4}T$', code_clean):
        return {"valid": True, "category": "Category III – Emerging Tech", "reason": ""}

    # ── HCPCS Level II ───────────────────────────────────────────────────
    if re.match(r'^[A-V]\d{4}$', code_clean):
        return {"valid": True, "category": "HCPCS Level II", "reason": ""}

    # ── Category I (5-digit numeric) ─────────────────────────────────────
    if re.match(r'^\d{5}$', code_clean):
        num = int(code_clean)

        if (100 <= num <= 1999) or (99100 <= num <= 99150):
            return {"valid": True, "category": "Anesthesia", "reason": ""}
        if 10000 <= num <= 19499:
            return {"valid": True, "category": "Surgery – Integumentary", "reason": ""}
        if 20000 <= num <= 29999:
            return {"valid": True, "category": "Surgery – Musculoskeletal", "reason": ""}
        if 30000 <= num <= 32999:
            return {"valid": True, "category": "Surgery – Respiratory", "reason": ""}
        if 33010 <= num <= 37799:
            return {"valid": True, "category": "Surgery – Cardiovascular", "reason": ""}
        if 40490 <= num <= 49999:
            return {"valid": True, "category": "Surgery – Digestive", "reason": ""}
        if 50010 <= num <= 53899:
            return {"valid": True, "category": "Surgery – Urinary", "reason": ""}
        if 61000 <= num <= 64999:
            return {"valid": True, "category": "Surgery – Nervous System", "reason": ""}
        if 10004 <= num <= 69990:            # catch-all for unlisted surgery sub-ranges
            return {"valid": True, "category": "Surgery", "reason": ""}
        if 70010 <= num <= 79999:
            return {"valid": True, "category": "Radiology", "reason": ""}
        if 80047 <= num <= 89398:
            return {"valid": True, "category": "Pathology / Lab", "reason": ""}
        if 99202 <= num <= 99499:
            return {"valid": True, "category": "Evaluation & Management", "reason": ""}
        if 90281 <= num <= 99607:
            return {"valid": True, "category": "Medicine", "reason": ""}

        return {
            "valid": False,
            "category": None,
            "reason": f"{code_clean} does not fall in any valid Category I range",
        }

    return {
        "valid": False,
        "category": None,
        "reason": f"Unrecognised CPT format: '{code_clean}' "
                  f"(expected 5-digit, 4+F, 4+T, or letter+4-digit)",
    }
...
class HybridExtractor:
    ...
    def extract_data(self, file_path: str) -> Dict[str, Any]:
        ...
        for page_name, page_data in qwen_result.items():
            ...
            # ── CPT code validation pass ──────────────────────────────────────
            # Run after OCR matching so the final cpt_codes list is already
            # enriched with bbox / ocr_confidence.  Invalid codes are flagged
            # with review_required = True regardless of llm_confidence.
            if "cpt_codes" in enriched_page:
                for cpt_item in enriched_page["cpt_codes"]:
                    raw_code = str(cpt_item.get("value", "")).strip()
                    v = validate_cpt_code(raw_code)
                    cpt_item["cpt_valid"]    = v["valid"]
                    cpt_item["cpt_category"] = v["category"]
                    cpt_item["cpt_reason"]   = v["reason"]
                    if not v["valid"]:
                        cpt_item["review_required"] = True
